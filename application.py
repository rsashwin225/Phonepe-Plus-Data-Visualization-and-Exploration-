...
import VizEngine.express as px
...
def data_overview():
    ...
    st.write('')
    st.header('PhonePe Pulse — Insights & Analysis (Ashwin)')
    st.write('')
    ...
    st.write('Transaction data broken down by type of payments at StateMapper level.')
    ...
    st.write('Users data broken down by devices at StateMapper level.')
    ...
    st.write('- Total number of transactions at the StateMapper / district level.')
    st.write('- Total value of all transactions at the StateMapper / district level.')

    st.subheader('Map User:')
    st.write('- Total number of registered users at the StateMapper / district level.')
    st.write('- Total number of app opens by these registered users at the StateMapper / district level.')
    ...
    st.write('- Top 10 Districts (by volume)')
    st.write('- Top 10 PINs (by volume)')
...
def state_list():
    db_conn = psycopg2.connect(
        host='localhost', user='postgres', password='root', database='phonepe_ashwin')
    cursor = db_conn.cursor()
    cursor.execute(f"""select distinct StateMapper from aggregated_transaction
                        order by StateMapper asc;""")
    s = cursor.fetchall()
    StateMapper = [i[0] for i in s]
    return StateMapper
...
def lookup_district_from_pincode(pincode):
...
class DataFetcher:
    ...
    def aggregated_transaction():
        try:
            path = "phonepe_pulse_git/data/aggregated/transaction/country/india/StateMapper/"
    ...
    def aggregated_user():
        try:
            path = "phonepe_pulse_git/data/aggregated/user/country/india/StateMapper/"
            agg_state_list = os.listdir(path)
    ...
    def map_transaction():
        try:
            path = "phonepe_pulse_git/data/map/transaction/hover/country/india/StateMapper/"
            agg_state_list = os.listdir(path)
    ...
    def map_user():
        try:
            path = "phonepe_pulse_git/data/map/user/hover/country/india/StateMapper/"
            agg_state_list = os.listdir(path)
    ...
    def top_transaction_district():
        try:
            path = "phonepe_pulse_git/data/top/transaction/country/india/StateMapper/"
            agg_state_list = os.listdir(path)
    ...
    def top_user_district():
        try:
            path = "phonepe_pulse_git/data/top/user/country/india/StateMapper/"
            agg_state_list = os.listdir(path)
...
class DataTransformer:
    ...
    aggregated_transaction = pd.DataFrame(DataFetcher.aggregated_transaction())
    aggregated_user = pd.DataFrame(DataFetcher.aggregated_user())
    map_transaction = pd.DataFrame(DataFetcher.map_transaction())
    map_user = pd.DataFrame(DataFetcher.map_user())
    top_transaction_district = pd.DataFrame(DataFetcher.top_transaction_district())
    top_transaction_pincode = pd.DataFrame(DataFetcher.top_transaction_pincode())
    top_user_district = pd.DataFrame(DataFetcher.top_user_district())
    top_user_pincode = pd.DataFrame(DataFetcher.top_user_pincode())


class DatabaseLoader:

    def sql_table_creation():
        db_conn = psycopg2.connect(host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute("create table if not exists aggregated_transaction(\
                            StateMapper				varchar(255),\
                            year				int,\
                            quater				varchar(2),\
    ...
    def data_migration():
        ...
        cursor.executemany("insert into aggregated_transaction(StateMapper, year, quater, transaction_type,\
                            transaction_count, transaction_amount)\
                           values(%s,%s,%s,%s,%s,%s)", DataTransformer.aggregated_transaction.values.tolist())
        
        cursor.executemany("insert into aggregated_user(StateMapper, year, quater, user_brand,\
                            user_count, user_percentage)\
                           values(%s,%s,%s,%s,%s,%s)", DataTransformer.aggregated_user.values.tolist())
        
        cursor.executemany("insert into map_transaction(StateMapper, year, quater, district,\
                            transaction_count, transaction_amount)\
                           values(%s,%s,%s,%s,%s,%s)", DataTransformer.map_transaction.values.tolist())
        
        cursor.executemany("insert into map_user(StateMapper, year, quater, district,\
                            registered_user, app_opens)\
                           values(%s,%s,%s,%s,%s,%s)", DataTransformer.map_user.values.tolist())
        
        cursor.executemany("insert into top_transaction_district(StateMapper, year, quater, district,\
                            transaction_count, transaction_amount)\
                           values(%s,%s,%s,%s,%s,%s)", DataTransformer.top_transaction_district.values.tolist())
        
        cursor.executemany("insert into top_transaction_pincode(StateMapper, year, quater, pincode,\
                            transaction_count, transaction_amount)\
                           values(%s,%s,%s,%s,%s,%s)", DataTransformer.top_transaction_pincode.values.tolist())
        
        cursor.executemany("insert into top_user_district(StateMapper, year, quater, district,registered_user)\
                           values(%s,%s,%s,%s,%s)", DataTransformer.top_user_district.values.tolist())
        
        cursor.executemany("insert into top_user_pincode(StateMapper, year, quater, pincode,registered_user)\
                           values(%s,%s,%s,%s,%s)", DataTransformer.top_user_pincode.values.tolist())
...
class ValueFormatter:
    ...
    def rupees(transaction):
        ...
        if len(b) <= 3:
            ...
            return ValueFormatter.thousands(b)
        elif len(b) in (7, 8, 9):
            return ValueFormatter.millions(b)
        elif len(b) in (10, 11, 12):
            return ValueFormatter.billions(b)
        elif len(b) >= 13:
            return ValueFormatter.trillions(b)
...
class StateMapper:
    ...
    def original_state_list():
        db_conn = psycopg2.connect(host='localhost', user='postgres',
                                password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select distinct StateMapper 
    ...
    def state_dict():
        original = StateMapper.original_state_list()
        geo = StateMapper.geo_state_list()
    ...
    def state_list(data):
        missed = set(StateMapper.original_state_list()
                     ).symmetric_difference(set(data))
        missed = list(missed)
        all_state = StateMapper.state_dict()
        if len(missed) > 0:
            for i in missed:
                del all_state[i]
...
class aggregated_transaction:
    ...
    def state_wise_total_transaction_count():
        db_conn = psycopg2.connect(host='localhost', user='postgres',
                                password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, sum(transaction_count) as transaction_count 
                            from aggregated_transaction
                            group by StateMapper
                            order by StateMapper asc;""")
        s = cursor.fetchall()
        ...
        data['State'] = StateMapper.state_list(state_original)
        data = data.sort_values(by='Transaction Count', ascending=False)
        data = data.rename_axis('S.No')
        data['Transaction'] = data['Transaction Count'].apply(lambda x: ValueFormatter.rupees(str(x)))
    ...
    def state_wise_total_transaction_amount():
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, sum(transaction_amount) as transaction_amount 
                            from aggregated_transaction
                            group by StateMapper
                            order by StateMapper asc;""")
        s = cursor.fetchall()
        ...
        data = data.rename_axis('S.No')
        data['Transaction'] = data['Transaction Amount'].apply(lambda x: ValueFormatter.rupees(str(x)))
        data.index = data.index.map(lambda x: '{:^{}}'.format(x, 10))
        return data
    ...
    def selectstate_year_wise_total_transaction_count(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, sum(transaction_count) as transaction_count 
                            from aggregated_transaction
                            where StateMapper=%s
                            group by year, StateMapper
    ...
    def selectstate_year_wise_total_transaction_amount(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, sum(transaction_amount) as transaction_amount 
                            from aggregated_transaction
                            where StateMapper=%s
                            group by year, StateMapper
                            order by year asc;""", (state_option,))
    ...
    def selectstate_quater_wise_total_transaction_count(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, sum(transaction_count) as transaction_count 
                        from aggregated_transaction
                        where StateMapper=%s
                        group by quater, StateMapper
    ...
    def selectstate_quater_wise_total_transaction_amount(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, sum(transaction_amount) as transaction_amount 
                            from aggregated_transaction
                            where StateMapper=%s
                            group by quater, StateMapper
                            order by quater asc;""", (state_option,))
    ...
    def selectstate_type_wise_total_transaction_count(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, transaction_type, sum(transaction_count) as transaction_count 
                        from aggregated_transaction
                        where StateMapper=%s
                        group by transaction_type, StateMapper
    ...
    def selectstate_type_wise_total_transaction_amount(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, transaction_type, sum(transaction_amount) as transaction_amount 
                        from aggregated_transaction
                        where StateMapper=%s
                        group by transaction_type, StateMapper
    ...
    def selectstate_year_quater_wise_total_transaction_count(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, sum(transaction_count) as transaction_count 
                        from aggregated_transaction
                        where StateMapper=%s
                        group by quater, year, StateMapper
    ...
    def selectstate_year_quater_wise_total_transaction_amount(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, sum(transaction_amount) as transaction_amount 
                            from aggregated_transaction
                            where StateMapper=%s
                            group by quater, year, StateMapper
                            order by year, quater asc;""", (state_option,))
    ...
    def selectstate_year_type_wise_total_transaction_count(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, transaction_type, sum(transaction_count) as transaction_count 
                        from aggregated_transaction
                        where StateMapper=%s
                        group by transaction_type, year, StateMapper
    ...
    def selectstate_year_type_wise_total_transaction_amount(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, transaction_type, sum(transaction_amount) as transaction_amount 
                            from aggregated_transaction
                            where StateMapper=%s
                            group by transaction_type, year, StateMapper
                            order by year asc, case when transaction_type = 'Others' then 1 else 0 end, transaction_type asc;""", (state_option,))
    ...
    def selectstate_quater_type_wise_total_transaction_count(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, transaction_type, sum(transaction_count) as transaction_count 
                        from aggregated_transaction
                        where StateMapper=%s
                        group by transaction_type, quater, StateMapper
    ...
    def selectstate_quater_type_wise_total_transaction_amount(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, transaction_type, sum(transaction_amount) as transaction_amount 
                            from aggregated_transaction
                            where StateMapper=%s
                            group by transaction_type, quater, StateMapper
                            order by quater asc, case when transaction_type = 'Others' then 1 else 0 end, transaction_type asc;""", (state_option,))
    ...
    def selectstate_selectyear_quater_wise_total_transaction_count(state_option, year_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, sum(transaction_count) as transaction_count 
                        from aggregated_transaction
                        where StateMapper=%s and year=%s
                        group by quater, year, StateMapper
        ...
        data['Transaction'] = data['Transaction Count'].apply(
            lambda x: ValueFormatter.rupees(str(x)))
        data.index = data.index.map(lambda x: '{:^{}}'.format(x, 10))
        return data
    ...
    def selectstate_selectyear_quater_type_wise_total_transaction_count(state_option, year_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, transaction_type, sum(transaction_count) as transaction_count 
                        from aggregated_transaction
                        where StateMapper=%s and year=%s
                        group by transaction_type, quater, year, StateMapper
    ...
    def selectstate_selectyear_quater_type_wise_total_transaction_amount(state_option, year_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, transaction_type, sum(transaction_amount) as transaction_amount 
                            from aggregated_transaction
                            where StateMapper=%s and year=%s
                            group by transaction_type, quater, year, StateMapper
    ...
    def selectstate_selectquater_year_wise_total_transaction_count(state_option, quater_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, sum(transaction_count) as transaction_count 
                        from aggregated_transaction
                        where StateMapper=%s and quater=%s
                        group by quater, year, StateMapper
    ...
    def selectstate_selectyear_selectquater_type_wise_total_transaction_count(state_option, year_option, quater_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, transaction_type, sum(transaction_count) as transaction_count 
                        from aggregated_transaction
                        where StateMapper=%s and year=%s and quater=%s
                        group by transaction_type, quater, year, StateMapper
    ...
    def selectyear_state_wise_total_transaction_count(year_option):
        ...
        cursor.execute(f"""select StateMapper, year, sum(transaction_count) as transaction_count 
                            ...
                            where year=%s
                            group by year, StateMapper
                            order by StateMapper asc;""", (year_option,))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(
    ...
    def selectyear_selectquater_state_wise_total_transaction_count(year_option, quater_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, sum(transaction_count) as transaction_count 
                            ...
                            group by quater, year, StateMapper
                            order by StateMapper asc;""", (year_option, quater_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(
    ...
    def selectquater_state_wise_total_transaction_count(quater_option):
        ...
        cursor.execute(f"""select StateMapper, quater, sum(transaction_count) as transaction_count 
                            ...
                            where quater=%s
                            group by quater, StateMapper
                            order by StateMapper asc;""", (quater_option,))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(
    ...
    def selecttype_state_wise_total_transaction_count(type_option):
        ...
        cursor.execute(f"""select StateMapper, transaction_type, sum(transaction_count) as transaction_count 
                            ...
                            where transaction_type=%s
                            group by transaction_type, StateMapper
                            order by StateMapper asc;""", (type_option,))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(
    ...
    def selectstate_selecttype_year_wise_total_transaction_count(state_option, type_option):
        ...
        cursor.execute(f"""select StateMapper, year, transaction_type, sum(transaction_count) as transaction_count 
                            from aggregated_transaction
                            where StateMapper=%s and transaction_type=%s
                            group by transaction_type, year, StateMapper
    ...
    def selectyear_selecttype_state_wise_total_transaction_count(year_option, type_option):
        ...
        cursor.execute(f"""select StateMapper, year, transaction_type, sum(transaction_count) as transaction_count 
                            ...
                            group by transaction_type, year, StateMapper
                            order by StateMapper asc;""", (year_option, type_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
    ...
    def selectquater_selecttype_state_wise_total_transaction_count(quater_option, type_option):
        ...
        cursor.execute(f"""select StateMapper, quater, transaction_type, sum(transaction_count) as transaction_count 
                            ...
                            group by transaction_type, quater, StateMapper
                            order by StateMapper asc;""", (quater_option, type_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(s, columns=[
    ...
    def selectstate_selectyear_selecttype_quater_wise_total_transaction_count(state_option, year_option, type_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, transaction_type, sum(transaction_count) as transaction_count 
                            from aggregated_transaction
                            where StateMapper=%s and year=%s and transaction_type=%s
                            group by transaction_type, quater, year, StateMapper
    ...
    def selectstate_selectquater_selecttype_year_wise_total_transaction_count(state_option, quater_option, type_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, transaction_type, sum(transaction_count) as transaction_count 
                            from aggregated_transaction
                            where StateMapper=%s and quater=%s and transaction_type=%s
                            group by transaction_type, quater, year, StateMapper
    ...
    def selectyear_selectquater_selecttype_state_wise_total_transaction_count(year_option, quater_option, type_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, transaction_type, sum(transaction_count) as transaction_count 
                            ...
                            group by transaction_type, quater, year, StateMapper
                            order by StateMapper asc;""", (year_option, quater_option, type_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(s, columns=[
    ...
    def selectstate_selectyear_selectquater_selecttype_wise_total_transaction_count(state_option, year_option, quater_option, type_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, transaction_type, sum(transaction_count) as transaction_count 
                        from aggregated_transaction
                        where StateMapper=%s and year=%s and quater=%s and transaction_type=%s
                        group by transaction_type, quater, year, StateMapper;""", (state_option, year_option, quater_option, type_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(
...
class aggregated_user:
    ...
    def state_wise_total_user_count():
        db_conn = psycopg2.connect(host='localhost', user='postgres',
                                password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, sum(user_count) as user_count 
    ...
    def selectyear_state_wise_total_user_count(year_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, sum(user_count) as user_count 
                                from aggregated_user
                                where year=%s
                                group by year, StateMapper
                                order by StateMapper asc;""", (year_option,))
    ...
    def selectquater_state_wise_total_user_count(quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, sum(user_count) as user_count 
                                from aggregated_user
                                where quater=%s
                                group by quater, StateMapper
                                order by StateMapper asc;""", (quater_option,))
    ...
    def selectbrand_state_wise_total_user_count(brand_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, user_brand, sum(user_count) as user_count 
                                from aggregated_user
                                where user_brand=%s
                                group by user_brand, StateMapper
                                order by StateMapper asc;""", (brand_option,))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(
    ...
    def selectstate_year_wise_total_user_count(state_option):
        ...
        data = data.rename_axis('S.No')
        data['User'] = data['User Count'].apply(lambda x: ValueFormatter.rupees(str(x)))
        data.index = data.index.map(lambda x: '{:^{}}'.format(x, 10))
        return data
    ...
    def selectstate_year_quater_wise_total_user_count(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, sum(user_count) as user_count 
                            from aggregated_user
                            where StateMapper=%s
                            group by quater, year, StateMapper
                            order by year, quater asc;""", (state_option,))
    ...
    def selectstate_selectyear_brand_wise_total_user_count(state_option, year_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, user_brand, sum(user_count) as user_count 
                            from aggregated_user
                            where StateMapper=%s and year=%s
                            group by user_brand, year, StateMapper
    ...
    def selectstate_selectquater_brand_wise_total_user_count(state_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, user_brand, sum(user_count) as user_count 
                            from aggregated_user
                            where StateMapper=%s and quater=%s
                            group by user_brand, quater, StateMapper
    ...
    def selectstate_selectyear_selectquater_brand_wise_total_user_count(state_option, year_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, user_brand, user_count 
                            from aggregated_user
                            where StateMapper=%s and year=%s and quater=%s
    ...
    def selectyear_selectstate_quater_brand_wise_total_user_count(state_option, year_option, quater_option):
        db_conn = psycopg2.connect(host='localhost', user='postgres',
                                password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, user_brand, sum(user_count) as user_count 
                                from aggregated_user
                                where StateMapper=%s and year=%s and quater=%s
                                group by user_brand, quater, year, StateMapper
                                order by case when user_brand = 'Others' then 1 else 0 end, user_count desc;""", (state_option, year_option, quater_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
    ...
    def selectstate_selectbrand_year_wise_total_user_count(state_option, brand_option):
        ...
        cursor.execute(f"""select StateMapper, year, user_brand, sum(user_count) as user_count 
                            from aggregated_user
                            where StateMapper=%s and user_brand=%s
                            group by user_brand, year, StateMapper
    ...
    def selectyear_selectbrand_state_wise_total_user_count(year_option, brand_option):
        ...
        cursor.execute(f"""select StateMapper, year, user_brand, sum(user_count) as user_count 
                            ...
                            group by user_brand, year, StateMapper
                            order by StateMapper asc;""", (year_option, brand_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(
    ...
    def selectquater_selectbrand_state_wise_total_user_count(quater_option, brand_option):
        ...
        cursor.execute(f"""select StateMapper, quater, user_brand, sum(user_count) as user_count 
                            ...
                            group by user_brand, quater, StateMapper
                            order by StateMapper asc;""", (quater_option, brand_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(
    ...
    def selectstate_selectyear_selectbrand_quater_wise_total_user_count(state_option, year_option, brand_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, user_brand, sum(user_count) as user_count 
                            from aggregated_user
                            where StateMapper=%s and year=%s and user_brand=%s
                            group by user_brand, quater, year, StateMapper
    ...
    def selectstate_selectquater_selectbrand_year_wise_total_user_count(state_option, quater_option, brand_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, user_brand, sum(user_count) as user_count 
                            from aggregated_user
                            where StateMapper=%s and quater=%s and user_brand=%s
                            group by user_brand, quater, year, StateMapper
    ...
    def selectyear_selectquater_selectbrand_state_wise_total_user_count(year_option, quater_option, brand_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, user_brand, sum(user_count) as user_count 
                            ...
                            group by user_brand, quater, year, StateMapper
                            order by StateMapper asc;""", (year_option, quater_option, brand_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(s, columns=[
    ...
    def selectstate_selectyear_selectquater_selectbrand_wise_total_user_count(state_option, year_option, quater_option, brand_option):
        ...
        cursor.execute(f"""select StateMapper, year, quater, user_brand, sum(user_count) as user_count 
                        from aggregated_user
                        where StateMapper=%s and year=%s and quater=%s and user_brand=%s
                        group by user_brand, quater, year, StateMapper;""", (state_option, year_option, quater_option, brand_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        pd.set_option('display.max_columns', None)
        data = pd.DataFrame(
...
class map_transaction:
    ...
    def selectstate_district_wise_total_transaction_count(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, district, sum(transaction_count) as transaction_count 
                            from map_transaction
                            where StateMapper=%s
                            group by district, StateMapper
        ...
        data = data.rename_axis('S.No')
        data['Formatted Value'] = data['Transaction Count'].apply(lambda x: ValueFormatter.rupees(str(x)))
        data.index = data.index.map(lambda x: '{:^{}}'.format(x, 10))
        return data
    ...
    def selectstate_district_wise_total_transaction_amount(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, district, sum(transaction_amount) as transaction_amount 
                            from map_transaction
                            where StateMapper=%s
                            group by district, StateMapper
        ...
        data['Transaction Amount'] = data['Transaction Amount'].apply(
            lambda x: int(round(x, 0)))
        data = data.rename_axis('S.No')
        data['Formatted Value'] = data['Transaction Amount'].apply(lambda x: ValueFormatter.rupees(str(x)))
        data.index = data.index.map(lambda x: '{:^{}}'.format(x, 10))
        return data
    ...
    def selectstate_selectyear_district_wise_total_transaction_count(state_option, year_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, district, sum(transaction_count) as transaction_count 
                            from map_transaction
                            where StateMapper=%s and year=%s
                            group by district, year, StateMapper
    ...
    def selectstate_selectyear_district_wise_total_transaction_amount(state_option, year_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, district, sum(transaction_amount) as transaction_amount 
                            from map_transaction
                            where StateMapper=%s and year=%s
                            group by district, year, StateMapper
    ...
    def selectstate_selectquater_district_wise_total_transaction_count(state_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, district, sum(transaction_count) as transaction_count 
                            from map_transaction
                            where StateMapper=%s and quater=%s
                            group by district, quater, StateMapper
    ...
    def selectstate_selectquater_district_wise_total_transaction_amount(state_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, district, sum(transaction_amount) as transaction_amount 
                            from map_transaction
                            where StateMapper=%s and quater=%s
                            group by district, quater, StateMapper
    ...
    def selectstate_selectyear_selectquater_district_wise_total_transaction_count(state_option, year_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, district, sum(transaction_count) as transaction_count 
                            from map_transaction
                            where StateMapper=%s and year=%s and quater=%s
                            group by district, quater, year, StateMapper
    ...
    def selectstate_selectyear_selectquater_district_wise_total_transaction_amount(state_option, year_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, district, sum(transaction_amount) as transaction_amount 
                            from map_transaction
                            where StateMapper=%s and year=%s and quater=%s
                            group by district, quater, year, StateMapper
...
class map_user:
    ...
    def state_wise_total_registered_user():
        db_conn = psycopg2.connect(host='localhost', user='postgres',
                                password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, sum(registered_user) as registered_user 
                                from map_user
                                group by StateMapper
                                order by StateMapper asc;""")
        s = cursor.fetchall()
    ...
    def state_wise_total_app_opens():
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, sum(app_opens) as app_opens 
                                from map_user
                                group by StateMapper
                                order by StateMapper asc;""")
        s = cursor.fetchall()
    ...
    def selectyear_state_wise_total_registered_user(year_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, sum(registered_user) as registered_user 
                                from map_user
                                where year=%s
                                group by year, StateMapper
                                order by StateMapper asc;""", (year_option,))
    ...
    def selectyear_state_wise_total_app_opens(year_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, sum(app_opens) as app_opens 
                                from map_user
                                where year=%s
                                group by year, StateMapper
                                order by StateMapper asc;""", (year_option,))
    ...
    def selectquater_state_wise_total_registered_user(quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, sum(registered_user) as registered_user 
                                from map_user
                                where quater=%s
                                group by quater, StateMapper
                                order by StateMapper asc;""", (quater_option,))
    ...
    def selectquater_state_wise_total_app_opens(quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, sum(app_opens) as app_opens 
                                from map_user
                                where quater=%s
                                group by quater, StateMapper
                                order by StateMapper asc;""", (quater_option,))
    ...
    def selectstate_year_wise_total_registered_user(state_option):
        ...
        data = data.rename_axis('S.No')
        data['Register'] = data['Registered Users'].apply(lambda x: ValueFormatter.rupees(str(x)))
        data.index = data.index.map(lambda x: '{:^{}}'.format(x, 10))
        return data
    ...
    def selectstate_year_wise_total_app_opens(state_option):
        ...
        data = data.rename_axis('S.No')
        data['App'] = data['App Opens'].apply(lambda x: ValueFormatter.rupees(str(x)))
        data.index = data.index.map(lambda x: '{:^{}}'.format(x, 10))
        return data
    ...
    def selectstate_year_quater_wise_total_registered_user(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, sum(registered_user) as registered_user 
                            from map_user
                            where StateMapper=%s
                            group by quater, year, StateMapper
                            order by year, quater asc;""", (state_option,))
    ...
    def selectstate_year_quater_wise_total_app_opens(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, sum(app_opens) as app_opens 
                                from map_user
                                where StateMapper=%s
                                group by quater, year, StateMapper
                                order by year, quater asc;""", (state_option,))
    ...
    def selectstate_district_wise_total_registered_user(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, district, sum(registered_user) as registered_user 
                                from map_user
                                where StateMapper=%s
                                group by district, StateMapper
        ...
        data = data.rename_axis('S.No')
        data['Formatted Value'] = data['Registered Users'].apply(lambda x: ValueFormatter.rupees(str(x)))
        data.index = data.index.map(lambda x: '{:^{}}'.format(x, 10))
        return data
    ...
    def selectstate_district_wise_total_app_opens(state_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, district, sum(app_opens) as app_opens 
                                from map_user
                                where StateMapper=%s
                                group by district, StateMapper
        ...
        data = data.rename_axis('S.No')
        data['Formatted Value'] = data['App Opens'].apply(lambda x: ValueFormatter.rupees(str(x)))
        data.index = data.index.map(lambda x: '{:^{}}'.format(x, 10))
        return data
    ...
    def selectstate_selectyear_district_wise_total_registered_user(state_option, year_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, district, sum(registered_user) as registered_user 
                                from map_user
                                where StateMapper=%s and year=%s
                                group by district, year, StateMapper
    ...
    def selectstate_selectyear_district_wise_total_app_opens(state_option, year_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, district, sum(app_opens) as app_opens 
                                from map_user
                                where StateMapper=%s and year=%s
                                group by district, year, StateMapper
    ...
    def selectstate_selectquater_district_wise_total_registered_user(state_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, district, sum(registered_user) as registered_user 
                                from map_user
                                where StateMapper=%s and quater=%s
                                group by district, quater, StateMapper
    ...
    def selectstate_selectquater_district_wise_total_app_opens(state_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, quater, district, sum(app_opens) as app_opens 
                                from map_user
                                where StateMapper=%s and quater=%s
                                group by district, quater, StateMapper
    ...
    def selectstate_selectyear_selectquater_district_wise_total_registered_user(state_option, year_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, district, sum(registered_user) as registered_user 
                                from map_user
                                where StateMapper=%s and year=%s and quater=%s
                                group by district, quater, year, StateMapper
    ...
    def selectstate_selectyear_selectquater_district_wise_total_app_opens(state_option, year_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, district, sum(app_opens) as app_opens 
                                from map_user
                                where StateMapper=%s and year=%s and quater=%s
                                group by district, quater, year, StateMapper
...
# transaction
    ...
    def selectyear_selectquater_state_wise_top10_transaction(year_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, sum(transaction_count) as transaction_count, sum(transaction_amount) as transaction_amount 
                            from aggregated_transaction
                            where year=%s and quater=%s
                            group by quater, year, StateMapper
        ...
        data['Transaction Amount'] = data['Transaction Amount'].apply(
            lambda x: int(round(x, 0)))
        data = data.rename_axis('S.No')
        data['Count'] = data['Transaction Count'].apply(lambda x: ValueFormatter.rupees(str(x)))
        data['Amount'] = data['Transaction Amount'].apply(lambda x: ValueFormatter.rupees(str(x)))
    ...
    def selectyear_selectquater_district_wise_top10_transaction(year_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, district, transaction_count, transaction_amount
                            from top_transaction_district
                            where year=%s and quater=%s
                            group by transaction_amount, transaction_count, district, quater, year, StateMapper
                            order by transaction_amount desc
                            limit 10;""", (year_option, quater_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
    ...
    def selectyear_selectquater_pincode_wise_top10_transaction(year_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, pincode, sum(transaction_count) as transaction_count, sum(transaction_amount) as transaction_amount
                            from top_transaction_pincode
                            where year=%s and quater=%s
                            group by pincode, quater, year, StateMapper
                            order by transaction_amount desc
                            limit 10;""", (year_option, quater_option))
        s = cursor.fetchall()
        i = [i for i in range(1, len(s)+1)]
        ...
        data['Avg. Transaction Value'] = data['Transaction Amount'] // data['Transaction Count']
        data.index = data.index.map(lambda x: '{:^{}}'.format(x, 10))
        data['District'] = data['Pincode'].apply(lambda x: lookup_district_from_pincode(x))
...
# user
    ...
    def selectyear_selectquater_district_wise_top10_user(year_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, district, registered_user
                            from top_user_district
                            where year=%s and quater=%s
                            group by registered_user, district, quater, year, StateMapper
    ...
    def selectyear_selectquater_pincode_wise_top10_user(year_option, quater_option):
        db_conn = psycopg2.connect(
            host='localhost', user='postgres', password='root', database='phonepe_ashwin')
        cursor = db_conn.cursor()
        cursor.execute(f"""select StateMapper, year, quater, pincode, sum(registered_user) as registered_user
        ...
        if analysis == 'State':
            ...
            with transactions:
                ...
                if option == 'Transaction Count':
                    ...
                    VizEngine.geo_map(state_wise_total_transaction_count, 'State',
                    ...
                    VizEngine.geo_map(state_wise_total_transaction_amount, 'State',
                ...
                if state_option and advanced_filters:
                    if year_option != 'Select One' and quater_option == 'Select One':
                        ...
                        with col1:
                            VizEngine.pie_chart(df=selectstate_selectyear_quater_wise_total_transaction_count, x='Quater', y='Transaction Count',
                                            title='Quater wise Transaction Count')
                        with col2:
                            VizEngine.pie_chart(df=selectstate_selectyear_quater_wise_total_transaction_amount, x='Quater', y='Transaction Amount',
                        ...
                        selectstate_selectyear_type_wise_total_transaction_amount = aggregated_transaction.selectstate_selectyear_type_wise_total_transaction_amount(
                                                                                                            state_option, year_option)
                        with col1:
                            VizEngine.horizontal_bar_chart(df=selectstate_selectyear_type_wise_total_transaction_count,y='Transaction Type',x='Transaction Count', 
                                                text='Transaction', color='#5D9A96', title='Type wise Transaction Count')
                        with col2:
                            VizEngine.horizontal_bar_chart(df=selectstate_selectyear_type_wise_total_transaction_amount,y='Transaction Type',x='Transaction Amount',
                        ...
                        selectstate_selectyear_quater_type_wise_total_transaction_amount = aggregated_transaction.selectstate_selectyear_quater_type_wise_total_transaction_amount(
                                                                                                                    state_option, year_option)
                        VizEngine.multi_line_chart(df=selectstate_selectyear_quater_type_wise_total_transaction_count, x='Quater', y='Transaction Count',
                                                    colorcolumn='Transaction Type', title='Quater - Type wise Transaction Count', title_x=0.38)
                        VizEngine.multi_line_chart(df=selectstate_selectyear_quater_type_wise_total_transaction_amount, x='Quater', y='Transaction Amount',
                        ...
                        selectstate_selectquater_year_wise_total_transaction_amount = aggregated_transaction.selectstate_selectquater_year_wise_total_transaction_amount(
                                                                                                            state_option, quater_option)
                        with col1:
                            VizEngine.pie_chart(df=selectstate_selectquater_year_wise_total_transaction_count, x='Year', y='Transaction Count',
                                            title='Year wise Transaction Count')
                        with col2:
                            VizEngine.pie_chart(df=selectstate_selectquater_year_wise_total_transaction_amount, x='Year', y='Transaction Amount',
                        ...
                        selectstate_selectquater_type_wise_total_transaction_amount = aggregated_transaction.selectstate_selectquater_type_wise_total_transaction_amount(
                                                                                                            state_option, quater_option)
                        with col1:
                            VizEngine.horizontal_bar_chart(df=selectstate_selectquater_type_wise_total_transaction_count,y='Transaction Type',x='Transaction Count', 
                                                        text='Transaction', color='#5D9A96', title='Type wise Transaction Count')
                        with col2:
                            VizEngine.horizontal_bar_chart(df=selectstate_selectquater_type_wise_total_transaction_amount,y='Transaction Type',x='Transaction Amount', 
                                                        text='Transaction', color='#5cb85c', title='Type wise Transaction Amount')
                        ...
                        selectstate_selectquater_year_type_wise_total_transaction_amount = aggregated_transaction.selectstate_selectquater_year_type_wise_total_transaction_amount(
                                                                                                                    state_option, quater_option)
                        VizEngine.multi_line_chart(df=selectstate_selectquater_year_type_wise_total_transaction_count, x='Year', y='Transaction Count',
                                                    colorcolumn='Transaction Type', title='Year - Type wise Transaction Count', title_x=0.40)
                        VizEngine.multi_line_chart(df=selectstate_selectquater_year_type_wise_total_transaction_amount, x='Year', y='Transaction Amount',
                        ...
                        selectstate_selectyear_selectquater_type_wise_total_transaction_amount = aggregated_transaction.selectstate_selectyear_selectquater_type_wise_total_transaction_amount(
                                                                                                                        state_option, year_option, quater_option)
                        with col1:
                            VizEngine.line_chart(df=selectstate_selectyear_selectquater_type_wise_total_transaction_count,y='Transaction Type',x='Transaction Count', 
                        ...
                        with col2:
                            VizEngine.line_chart(df=selectstate_selectyear_selectquater_type_wise_total_transaction_amount,y='Transaction Type',x='Transaction Amount', 
                    ...
                    with col1:
                        VizEngine.vertical_bar_chart(df=selectstate_year_wise_total_transaction_count,x='Year',y='Transaction Count', 
                                                    text='Transaction', color='#5D9A96', title='Year wise Transaction Count')
                    with col2:
                        VizEngine.vertical_bar_chart(df=selectstate_year_wise_total_transaction_amount,x='Year',y='Transaction Amount', 
                    ...
                    with col1:
                        VizEngine.pie_chart(df=selectstate_quater_wise_total_transaction_count, x='Quater', y='Transaction Count',
                                        title='Quater wise Transaction Count')
                    with col2:
                        VizEngine.pie_chart(df=selectstate_quater_wise_total_transaction_amount, x='Quater', y='Transaction Amount',
                                        title='Quater wise Transaction Amount')
                    ...
                    with col1:
                        VizEngine.line_chart(df=selectstate_type_wise_total_transaction_count,y='Transaction Type',x='Transaction Count', 
                                            text='Transaction', textposition=['bottom left','top right','top right','top right','top right'],
                                              color='#ba6e77', title='Type wise Transaction Count', title_x=0.35)
                    with col2:
                        VizEngine.line_chart(df=selectstate_type_wise_total_transaction_amount,y='Transaction Type',x='Transaction Amount', 
                                            text='Transaction', textposition=['bottom left','top right','top right','top right','top right'], 
                    ...
                    with col1:
                        VizEngine.multi_line_chart(df=selectstate_year_quater_wise_total_transaction_count, x='Quater', y='Transaction Count',
                                                colorcolumn='Year', title='Year - Quater wise Transaction Count')                                            
                    with col2:
                        VizEngine.multi_line_chart(df=selectstate_year_quater_wise_total_transaction_amount, x='Quater', y='Transaction Amount',
                                                colorcolumn='Year', title='Year - Quater wise Transaction Amount')                                     
                    
                    VizEngine.multi_line_chart(df=selectstate_year_type_wise_total_transaction_count, y='Transaction Count', x='Year',
                                                colorcolumn='Transaction Type', title='Year - Type wise Transaction Count')
                    VizEngine.multi_line_chart(df=selectstate_year_type_wise_total_transaction_amount, y='Transaction Amount', x='Year',
                                                colorcolumn='Transaction Type', title='Year - Type wise Transaction Amount')
                    
                    col1, col2 = st.columns(2)
                    with col1:                   
                        VizEngine.multi_line_chart(df=selectstate_quater_type_wise_total_transaction_count, x='Quater', y='Transaction Count',
                                                colorcolumn='Transaction Type', title='Quater - Type wise Transaction Count')
                    with col2:               
                        VizEngine.multi_line_chart(df=selectstate_quater_type_wise_total_transaction_amount, x='Quater', y='Transaction Amount',
            ...
            with users:
                ...
                if option == 'User Count':
                    ...
                    VizEngine.geo_map(state_wise_total_user_count,
                    ...
                    VizEngine.geo_map(state_wise_total_registered_user, 'State',
                    ...
                    VizEngine.geo_map(state_wise_total_app_opens,
                ...
                if state_option and advanced_filters:
                    if year_option != 'Select One' and quater_option == 'Select One':
                        ...
                        selectstate_selectyear_quater_wise_total_user_count = aggregated_user.selectstate_selectyear_quater_wise_total_user_count(
                                                                                                state_option, year_option)
                        VizEngine.pie_chart(df=selectstate_selectyear_quater_wise_total_user_count, x='Quater', y='User Count',
                        ...
                        selectstate_selectyear_brand_wise_total_user_count = aggregated_user.selectstate_selectyear_brand_wise_total_user_count(
                                                                                                state_option, year_option)
                        VizEngine.vertical_bar_chart(df=selectstate_selectyear_brand_wise_total_user_count,x='User Brand',y='User Count', 
                        ...
                        selectstate_selectyear_quater_brand_wise_total_user_count = aggregated_user.selectstate_selectyear_quater_brand_wise_total_user_count(
                                                                                                        state_option, year_option)
                        VizEngine.multi_line_chart(df=selectstate_selectyear_quater_brand_wise_total_user_count, x='Quater', y='User Count',
                        ...
                        selectstate_selectyear_quater_wise_total_app_opens = map_user.selectstate_selectyear_quater_wise_total_app_opens(
                                                                                        state_option, year_option)
                        with col1:
                            VizEngine.line_chart(df=selectstate_selectyear_quater_wise_total_registered_user,x='Quater',y='Registered Users', 
                        ...
                        with col2:
                            VizEngine.line_chart(df=selectstate_selectyear_quater_wise_total_app_opens,x='Quater',y='App Opens', 
                        ...
                        selectstate_selectquater_year_wise_total_user_count = aggregated_user.selectstate_selectquater_year_wise_total_user_count(
                                                                                                state_option, quater_option)
                        VizEngine.pie_chart(df=selectstate_selectquater_year_wise_total_user_count, x='Year', y='User Count',
                        ...
                        selectstate_selectquater_brand_wise_total_user_count = aggregated_user.selectstate_selectquater_brand_wise_total_user_count(
                                                                                                state_option, quater_option)
                        VizEngine.vertical_bar_chart(df=selectstate_selectquater_brand_wise_total_user_count,x='User Brand',y='User Count', 
                                                    text='User', color='#5D9A96', title='Brand wise User Count', title_x=0.45)
                        ...
                        selectstate_selectquater_year_brand_wise_total_user_count = aggregated_user.selectstate_selectquater_year_brand_wise_total_user_count(
                                                                                                    state_option, quater_option)
                        VizEngine.multi_line_chart(df=selectstate_selectquater_year_brand_wise_total_user_count, x='Year', y='User Count',
                        ...
                        with col1:
                            VizEngine.line_chart(df=selectstate_selectquater_year_wise_total_registered_user,x='Year',y='Registered Users', 
                        ...
                        with col2:
                            VizEngine.line_chart(df=selectstate_selectquater_year_wise_total_app_opens,x='Year',y='App Opens', 
                        ...
                        selectstate_selectyear_selectquater_brand_wise_total_user_count = aggregated_user.selectstate_selectyear_selectquater_brand_wise_total_user_count(
                                                                                                state_option, year_option, quater_option)
                        VizEngine.vertical_bar_chart(df=selectstate_selectyear_selectquater_brand_wise_total_user_count,x='User Brand',y='User Count', 
                                                    text='User', color='#5D9A96', title='Brand wise User Count', title_x=0.45)
                    ...
                    # line chart
                        ...
                        VizEngine.line_chart(df=selectstate_quater_wise_total_user_count,y='Quater',x='User Count', 
                    ...
                    # pie chart
                        ...
                        VizEngine.pie_chart(df=selectstate_year_wise_total_user_count, x='Year', y='User Count',
                    ...
                    VizEngine.vertical_bar_chart(df=selectstate_brand_wise_total_user_count,x='User Brand',y='User Count', 
                    ...
                    VizEngine.marker_multi_line_chart(df=selectstate_year_quater_wise_total_user_count, x='Quater', y='User Count',
                    ...
                    with col1:
                        VizEngine.vertical_bar_chart(df=selectstate_year_wise_total_registered_user,x='Year',y='Registered Users', 
                                                    text='Register', color='#5D9A96', title='Year wise Registered Users', title_x=0.30)
                    with col2:
                        VizEngine.vertical_bar_chart(df=selectstate_year_wise_total_app_opens,x='Year',y='App Opens', 
                    ...
                    with col1:
                        VizEngine.pie_chart(df=selectstate_quater_wise_total_registered_user, x='Quater', y='Registered Users',
                                        title='Quater wise Registered Users')
                    with col2:
                        VizEngine.pie_chart(df=selectstate_quater_wise_total_app_opens, x='Quater', y='App Opens',
                    ...
                    with col1:
                        VizEngine.multi_line_chart(df=selectstate_year_quater_wise_total_registered_user, x='Quater', y='Registered Users',
                                                colorcolumn='Year',title='Year - Quater wise Registered Users')

                    with col2:
                        VizEngine.multi_line_chart(df=selectstate_year_quater_wise_total_app_opens, x='Quater', y='App Opens',
            ...
            with transactions:
                ...
                with col1:
                    VizEngine.vertical_bar_chart(df=year_wise_total_transaction_count,x='Year',y='Transaction Count', 
                                                text='Transaction', color='#5D9A96', title='Year wise Transaction Count')
                with col2:
                    VizEngine.vertical_bar_chart(df=year_wise_total_transaction_amount,x='Year',y='Transaction Amount', 
                                                text='Transaction', color='#5cb85c', title='Year wise Transaction Amount')
                ...
                with col1:
                    VizEngine.multi_line_chart(df=year_quater_wise_total_transaction_count, x='Quater', y='Transaction Count',
                                                colorcolumn='Year', title='Year - Quater wise Transaction Count')
                with col2:
                    VizEngine.multi_line_chart(df=year_quater_wise_total_transaction_amount, x='Quater', y='Transaction Amount',
                                            colorcolumn='Year', title='Year - Quater wise Transaction Amount')
                ...
                VizEngine.multi_line_chart(df=year_type_wise_total_transaction_count, y='Transaction Count', x='Year',
                                        colorcolumn='Transaction Type', title='Year - Type wise Transaction Count', title_x=0.35)
                VizEngine.multi_line_chart(df=year_type_wise_total_transaction_amount, y='Transaction Amount', x='Year',
                ...
                if year_option and advanced_filters:
                    if state_option != 'Select One' and quater_option == 'Select One':
                        ...
                        if option == 'Transaction Count':
                            ...
                            VizEngine.geo_map(selectyear_selectquater_state_wise_total_transaction_count,
                            ...
                            selectyear_selectquater_state_wise_total_transaction_amount = aggregated_transaction.selectyear_selectquater_state_wise_total_transaction_amount(
                                year_option, quater_option)
                            VizEngine.geo_map(selectyear_selectquater_state_wise_total_transaction_amount,
                        ...
                        selectyear_selectquater_type_wise_total_transaction_amount = aggregated_transaction.selectyear_selectquater_type_wise_total_transaction_amount(
                                                                                                            year_option, quater_option)
                        with col1:
                            VizEngine.line_chart(df=selectyear_selectquater_type_wise_total_transaction_count,y='Transaction Type',x='Transaction Count', 
                                                text='Transaction', textposition=['bottom left','top right','top right','top right','top right'], 
                                                color='#ba6e77', title='Type wise Transaction Count', title_x=0.35)
                        with col2:
                            VizEngine.line_chart(df=selectyear_selectquater_type_wise_total_transaction_amount,y='Transaction Type',x='Transaction Amount', 
                                                text='Transaction', textposition=['bottom left','top right','top right','top right','top right'], 
                                                color='#716cf8', title='Type wise Transaction Amount', title_x=0.35)
                    ...
                    if option == 'Transaction Count':
                        ...
                        VizEngine.geo_map(selectyear_state_wise_total_transaction_count,
                                       'State', 'Transaction Count', 'State wise Transaction Count')
                    elif option == 'Transaction Amount':
                        ...
                        VizEngine.geo_map(selectyear_state_wise_total_transaction_amount,
                                       'State', 'Transaction Amount', 'State wise Transaction Amount')
                    ...
                    with col1:
                        VizEngine.pie_chart(df=selectyear_quater_wise_total_transaction_count, x='Quater', y='Transaction Count',
                                        title='Quater wise Transaction Count')
                    with col2:
                        VizEngine.pie_chart(df=selectyear_quater_wise_total_transaction_amount, x='Quater', y='Transaction Amount',
                                        title='Quater wise Transaction Amount')
                    ...
                    with col1:
                        VizEngine.horizontal_bar_chart(df=selectyear_type_wise_total_transaction_count,y='Transaction Type',x='Transaction Count', 
                                                    text='Transaction', color='#5D9A96', title='Type wise Transaction Count')
                    with col2:
                        VizEngine.horizontal_bar_chart(df=selectyear_type_wise_total_transaction_amount,y='Transaction Type',x='Transaction Amount', 
                                                    text='Transaction', color='#5cb85c', title='Type wise Transaction Amount')
                    ...
                    VizEngine.multi_line_chart(df=selectyear_quater_type_wise_total_transaction_count, y='Transaction Count',x='Quater',
                                                colorcolumn='Transaction Type', title='Quater - Type wise Transaction Count', title_x=0.35)
                    VizEngine.multi_line_chart(df=selectyear_quater_type_wise_total_transaction_amount, y='Transaction Amount',x='Quater',
                                                colorcolumn='Transaction Type', title='Quater - Type wise Transaction Amount', title_x=0.35)
                    
            with users:
                # vertical_bar chart
                year_wise_total_user_count = aggregated_user.year_wise_total_user_count()
                VizEngine.vertical_bar_chart(df=year_wise_total_user_count,x='Year',y='User Count', 
                ...
                VizEngine.marker_multi_line_chart(df=year_quater_wise_total_user_count, x='Quater', y='User Count',
                ...
                with col1:
                    VizEngine.pie_chart(df=year_wise_total_registered_user, x='Year', y='Registered Users',
                                    title='Year wise Registered Users')
                with col2:
                    VizEngine.pie_chart(df=year_wise_total_app_opens, x='Year', y='App Opens',
                ...
                with col1:
                    VizEngine.multi_line_chart(df=year_quater_wise_total_registered_user, x='Quater', y='Registered Users',
                                                colorcolumn='Year', title='Year - Quater wise Registered Users')
                with col2:
                    VizEngine.multi_line_chart(df=year_quater_wise_total_app_opens, x='Quater', y='App Opens',
                                            colorcolumn='Year', title='Year - Quater wise App Opens')
                ...
                if year_option and advanced_filters:
                    if state_option != 'Select One' and quater_option == 'Select One':
                        ...
                        selectstate_selectyear_quater_wise_total_user_count = aggregated_user.selectstate_selectyear_quater_wise_total_user_count(
                                                                                            state_option, year_option)
                        VizEngine.line_chart(df=selectstate_selectyear_quater_wise_total_user_count,x='Quater',y='User Count', 
                        ...
                        selectyear_selectstate_brand_wise_total_user_count = aggregated_user.selectyear_selectstate_brand_wise_total_user_count(
                                                                                                state_option, year_option)
                        VizEngine.vertical_bar_chart(df=selectyear_selectstate_brand_wise_total_user_count,x='User Brand',y='User Count', 
                        ...
                        selectstate_selectyear_quater_wise_total_app_opens = map_user.selectstate_selectyear_quater_wise_total_app_opens(
                                                                                        state_option, year_option)
                        with col1:
                            VizEngine.pie_chart(df=selectstate_selectyear_quater_wise_total_registered_user, x='Quater', y='Registered Users',
                                            title='Quater wise Registered Users')
                        with col2:
                            VizEngine.pie_chart(df=selectstate_selectyear_quater_wise_total_app_opens, x='Quater', y='App Opens',
                                            title='Quater wise App Opens')
                        ...
                        if option == 'User Count':
                            selectyear_selectquater_state_wise_total_user_count = aggregated_user.selectyear_selectquater_state_wise_total_user_count(
                                year_option, quater_option)
                            VizEngine.geo_map(selectyear_selectquater_state_wise_total_user_count,
                            ...
                            selectyear_selectquater_state_wise_total_registered_user = map_user.selectyear_selectquater_state_wise_total_registered_user(
                                year_option, quater_option)
                            VizEngine.geo_map(selectyear_selectquater_state_wise_total_registered_user,
                            ...
                            selectyear_selectquater_state_wise_total_app_opens = map_user.selectyear_selectquater_state_wise_total_app_opens(
                                year_option, quater_option)
                            VizEngine.geo_map(selectyear_selectquater_state_wise_total_app_opens,
                        ...
                        selectyear_selectquater_brand_wise_total_user_count = aggregated_user.selectyear_selectquater_brand_wise_total_user_count(
                                                                                                year_option, quater_option)
                        VizEngine.vertical_bar_chart(df=selectyear_selectquater_brand_wise_total_user_count, x='User Brand', y='User Count', 
                                                    text='User', color='#5D9A96', title='Brand wise User Count', title_x=0.40)
                    ...
                    if option == 'User Count':
                        selectyear_state_wise_total_user_count = aggregated_user.selectyear_state_wise_total_user_count(
                                                                                year_option)
                        VizEngine.geo_map(selectyear_state_wise_total_user_count,
                        ...
                        selectyear_state_wise_total_registered_user = map_user.selectyear_state_wise_total_registered_user(
                                                                                year_option)
                        VizEngine.geo_map(selectyear_state_wise_total_registered_user,
                                       'State', 'Registered Users', 'State wise Registered Users')

                    elif option == 'App Opens':
                        selectyear_state_wise_total_app_opens = map_user.selectyear_state_wise_total_app_opens(
                                                                        year_option)
                        VizEngine.geo_map(selectyear_state_wise_total_app_opens,
                    ...
                    VizEngine.line_chart(df=selectyear_quater_wise_total_user_count,x='Quater',y='User Count', 
                                        text='User', textposition='top left', color='#ba6e77', title='Quater wise User Count', title_x=0.40)
                    
                    # vertical_bar chart
                    selectyear_brand_wise_total_user_count = aggregated_user.selectyear_brand_wise_total_user_count(year_option)
                    VizEngine.vertical_bar_chart(df=selectyear_brand_wise_total_user_count,x='User Brand',y='User Count', 
                                                text='User', color='#5D9A96', title='Brand wise User Count', title_x=0.40)
                    
                    # multi line chart
                    selectyear_quater_brand_wise_total_user_count = aggregated_user.selectyear_quater_brand_wise_total_user_count(year_option)
                    VizEngine.multi_line_chart(df=selectyear_quater_brand_wise_total_user_count, y='User Count', x='Quater',
                                                colorcolumn='User Brand', title='Quater - Brand wise User Count', title_x=0.40)
                    ...
                    with col1:
                        VizEngine.pie_chart(df=selectyear_quater_wise_total_registered_user, x='Quater', y='Registered Users',
                                        title='Quater wise Registered Users')
                    with col2:
                        VizEngine.pie_chart(df=selectyear_quater_wise_total_app_opens, x='Quater', y='App Opens',
                                        title='Quater wise App Opens')
            ...
            with transactions:
                ...
                with col1:
                    VizEngine.vertical_bar_chart(df=quater_wise_total_transaction_count,x='Quater',y='Transaction Count', 
                                                text='Transaction', color='#5D9A96', title='Quater wise Transaction Count')
                with col2:
                    VizEngine.vertical_bar_chart(df=quater_wise_total_transaction_amount,x='Quater',y='Transaction Amount', 
                ...
                year_quater_wise_total_transaction_amount = aggregated_transaction.year_quater_wise_total_transaction_amount()
                with col1:
                    VizEngine.multi_line_chart(df=year_quater_wise_total_transaction_count, x='Year', y='Transaction Count',
                                                colorcolumn='Quater', title='Year - Quater wise Transaction Count')
                with col2:
                    VizEngine.multi_line_chart(df=year_quater_wise_total_transaction_amount, x='Year', y='Transaction Amount',
                ...
                VizEngine.multi_line_chart(df=quater_type_wise_total_transaction_count, y='Transaction Count', x='Quater',
                                        colorcolumn='Transaction Type', title='Quater - Type wise Transaction Count', title_x=0.35)
                VizEngine.multi_line_chart(df=quater_type_wise_total_transaction_amount, y='Transaction Amount', x='Quater',
                                        colorcolumn='Transaction Type', title='Quater - Type wise Transaction Amount', title_x=0.35)
                ...
                if quater_option and advanced_filters:
                    if state_option != 'Select One' and year_option == 'Select One':
                        ...
                        selectstate_selectquater_year_type_wise_total_transaction_amount = aggregated_transaction.selectstate_selectquater_year_type_wise_total_transaction_amount(
                                                                                                                    state_option, quater_option)
                        VizEngine.multi_line_chart(df=selectstate_selectquater_year_type_wise_total_transaction_count, y='Transaction Count', x='Year',
                                                colorcolumn='Transaction Type', title='Year - Type wise Transaction Count', title_x=0.35)
                        VizEngine.multi_line_chart(df=selectstate_selectquater_year_type_wise_total_transaction_amount, y='Transaction Amount', x='Year',
                                                colorcolumn='Transaction Type', title='Year - Type wise Transaction Amount', title_x=0.35)
                        ...
                        selectyear_selectquater_type_wise_total_transaction_amount = aggregated_transaction.selectyear_selectquater_type_wise_total_transaction_amount(
                            year_option, quater_option)
                        with col1:
                            VizEngine.horizontal_bar_chart(df=selectyear_selectquater_type_wise_total_transaction_count,y='Transaction Type',x='Transaction Count', 
                                                        text='Transaction', color='#5D9A96', title='Type wise Transaction Count')
                        with col2:
                            VizEngine.horizontal_bar_chart(df=selectyear_selectquater_type_wise_total_transaction_amount,y='Transaction Type',x='Transaction Amount', 
                                                        text='Transaction', color='#5cb85c', title='Type wise Transaction Amount')
                        ...
                        selectstate_selectyear_selectquater_type_wise_total_transaction_amount = aggregated_transaction.selectstate_selectyear_selectquater_type_wise_total_transaction_amount(
                                                                                                                        state_option, year_option, quater_option)
                        with col1:
                            VizEngine.horizontal_bar_chart(df=selectstate_selectyear_selectquater_type_wise_total_transaction_count,y='Transaction Type',x='Transaction Count', 
                                                        text='Transaction', color='#5D9A96', title='Type wise Transaction Count')
                        with col2:
                            VizEngine.horizontal_bar_chart(df=selectstate_selectyear_selectquater_type_wise_total_transaction_amount,y='Transaction Type',x='Transaction Amount', 
                                                        text='Transaction', color='#5cb85c', title='Type wise Transaction Amount')
                    ...
                    if option == 'Transaction Count':
                        selectquater_state_wise_total_transaction_count = aggregated_transaction.selectquater_state_wise_total_transaction_count(
                            quater_option)
                        VizEngine.geo_map(selectquater_state_wise_total_transaction_count,
                                       'State', 'Transaction Count', 'State wise Transaction Count')

                    elif option == 'Transaction Amount':
                        selectquater_state_wise_total_transaction_amount = aggregated_transaction.selectquater_state_wise_total_transaction_amount(
                            quater_option)
                        VizEngine.geo_map(selectquater_state_wise_total_transaction_amount,
                                       'State', 'Transaction Amount', 'State wise Transaction Amount')
                    # vertical_bar chart
                    col1, col2 = st.columns(2)
                    ...
                    with col1:
                        VizEngine.vertical_bar_chart(df=selectquater_year_wise_total_transaction_count,x='Year',y='Transaction Count', 
                                                    text='Transaction', color='#5D9A96', title='Year wise Transaction Count')
                    with col2:
                        VizEngine.vertical_bar_chart(df=selectquater_year_wise_total_transaction_amount,x='Year',y='Transaction Amount', 
                                                    text='Transaction', color='#5cb85c', title='Year wise Transaction Amount')
                    ...
                    with col1:
                        VizEngine.line_chart(df=selectquater_type_wise_total_transaction_count,y='Transaction Type',x='Transaction Count', 
                                            text='Transaction', textposition=['bottom left','bottom left','top right','top right','top right'], 
                                            color='#ba6e77', title='Type wise Transaction Count')
                    with col2:
                        VizEngine.line_chart(df=selectquater_type_wise_total_transaction_amount,y='Transaction Type',x='Transaction Amount', 
                                            text='Transaction', textposition=['top center','top right','top right','top right','top right'], 
                    ...
                    selectquater_year_type_wise_total_transaction_amount = aggregated_transaction.selectquater_year_type_wise_total_transaction_amount(
                                                                                                    quater_option)
                    VizEngine.multi_line_chart(df=selectquater_year_type_wise_total_transaction_count, y='Transaction Count',x='Year',
                                            colorcolumn='Transaction Type', title='Year - Type wise Transaction Count', title_x=0.35)
                    VizEngine.multi_line_chart(df=selectquater_year_type_wise_total_transaction_amount, y='Transaction Amount',x='Year',
                                            colorcolumn='Transaction Type', title='Year - Type wise Transaction Amount', title_x=0.35)
            ...
            with users:
                ...
                with col1:
                    ...
                    VizEngine.vertical_bar_chart(df=quater_wise_total_user_count,x='Quater',y='User Count', 
                                                text='User', color='#5D9A96', title='Quater wise User Count', title_x=0.40)
                with col2:
                    # marker multi line chart
                    year_quater_wise_total_user_count = aggregated_user.year_quater_wise_total_user_count()
                    VizEngine.multi_line_chart(df=year_quater_wise_total_user_count, x='Year', y='User Count',
                ...
                with col1:
                    VizEngine.pie_chart(df=quater_wise_total_registered_user, x='Quater', y='Registered Users',
                                    title='Quater wise Registered Users')
                with col2:
                    VizEngine.pie_chart(df=quater_wise_total_app_opens, x='Quater', y='App Opens',
                                    title='Quater wise App Opens')
                ...
                year_quater_wise_total_app_opens = map_user.year_quater_wise_total_app_opens()
                with col1:
                    VizEngine.multi_line_chart(df=year_quater_wise_total_registered_user, x='Year', y='Registered Users',
                                                colorcolumn='Quater', title='Year - Quater wise Registered Users')
                with col2:
                    VizEngine.multi_line_chart(df=year_quater_wise_total_app_opens, x='Year', y='App Opens',
                ...
                if quater_option and advanced_filters:
                    if state_option != 'Select One' and year_option == 'Select One':
                        ...
                        selectstate_selectquater_year_wise_total_user_count = aggregated_user.selectstate_selectquater_year_wise_total_user_count(
                                                                                                state_option, quater_option)
                        VizEngine.line_chart(df=selectstate_selectquater_year_wise_total_user_count,x='Year',y='User Count', 
                        ...
                        selectstate_selectquater_year_brand_wise_total_user_count = aggregated_user.selectstate_selectquater_year_brand_wise_total_user_count(
                                                                                                    state_option, quater_option)
                        VizEngine.multi_line_chart(df=selectstate_selectquater_year_brand_wise_total_user_count, y='User Count', x='Year',
                                                    colorcolumn='User Brand', title='Year - Brand wise User Count', title_x=0.40)
                        ...
                        selectstate_selectquater_year_wise_total_app_opens = map_user.selectstate_selectquater_year_wise_total_app_opens(
                                                                                        state_option, quater_option)
                        with col1:
                            VizEngine.pie_chart(df=selectstate_selectquater_year_wise_total_registered_user, x='Year', y='Registered Users',
                                            title='Year wise Registered Users')
                        with col2:
                            VizEngine.pie_chart(df=selectstate_selectquater_year_wise_total_app_opens, x='Year', y='App Opens',
                                            title='Year wise App Opens')
                    ...
                    VizEngine.multi_line_chart(df=selectquater_year_brand_wise_total_user_count, y='User Count', x='Year',
                                                colorcolumn='User Brand', title='Year - Brand wise User Count', title_x=0.40)
                    ...
                    with col1:
                        VizEngine.pie_chart(df=selectquater_year_wise_total_registered_user, x='Year', y='Registered Users',
                                        title='Year wise Registered Users')
                    with col2:
                        VizEngine.pie_chart(df=selectquater_year_wise_total_app_opens, x='Year', y='App Opens',
                                        title='Year wise App Opens')
            ...
            with col1:
                VizEngine.horizontal_bar_chart(df=type_wise_total_transaction_count,y='Transaction Type',x='Transaction Count', 
                                            text='Transaction', color='#5D9A96', title='Type wise Transaction Count')
            with col2:
                VizEngine.horizontal_bar_chart(df=type_wise_total_transaction_amount,y='Transaction Type',x='Transaction Amount', 
                                            text='Transaction', color='#5cb85c', title='Type wise Transaction Amount')
            ...
            VizEngine.multi_line_chart(df=type_year_wise_total_transaction_count, y='Transaction Count',x='Year',
                                        colorcolumn='Transaction Type', title='Year - Type wise Transaction Count', title_x=0.35)
            VizEngine.multi_line_chart(df=type_year_wise_total_transaction_amount, y='Transaction Amount',x='Year',
                                        colorcolumn='Transaction Type', title='Year - Type wise Transaction Amount', title_x=0.35)
            ...
            with col1:
                VizEngine.multi_line_chart(df=type_quater_wise_total_transaction_count, y='Transaction Count',x='Quater',
                                        colorcolumn='Transaction Type', title='Quater - Type wise Transaction Count')
            with col2:
                VizEngine.multi_line_chart(df=type_quater_wise_total_transaction_amount, y='Transaction Amount',x='Quater',
                                        colorcolumn='Transaction Type', title='Quater - Type wise Transaction Amount')
            ...
            if type_option and advanced_filters:
                ...
                if state_option != 'Select One' and year_option == 'Select One' and quater_option == 'Select One':
                    ...
                    selectstate_selecttype_year_wise_total_transaction_amount = aggregated_transaction.selectstate_selecttype_year_wise_total_transaction_amount(
                                                                                                        state_option, type_option)
                    with col1:
                        VizEngine.vertical_bar_chart(df=selectstate_selecttype_year_wise_total_transaction_count,x='Year',y='Transaction Count', 
                                                    text='Transaction', color='#5D9A96', title='Year wise Transaction Count')
                    with col2:
                        VizEngine.vertical_bar_chart(df=selectstate_selecttype_year_wise_total_transaction_amount,x='Year',y='Transaction Amount', 
                                                    text='Transaction', color='#5cb85c', title='Year wise Transaction Amount')
                    ...
                    selectstate_selecttype_quater_wise_total_transaction_amount = aggregated_transaction.selectstate_selecttype_quater_wise_total_transaction_amount(
                                                                                                            state_option, type_option)
                    with col1:
                        VizEngine.pie_chart(df=selectstate_selecttype_quater_wise_total_transaction_count, x='Quater', y='Transaction Count',
                                        title='Quater wise Transaction Count')
                    with col2:
                        VizEngine.pie_chart(df=selectstate_selecttype_quater_wise_total_transaction_amount, x='Quater', y='Transaction Amount',
                                        title='Quater wise Transaction Amount') 
                    ...
                    selectstate_selecttype_year_quater_wise_total_transaction_amount = aggregated_transaction.selectstate_selecttype_year_quater_wise_total_transaction_amount(
                                                                                                                state_option, type_option)
                    with col1:
                        VizEngine.multi_line_chart(df=selectstate_selecttype_year_quater_wise_total_transaction_count, y='Transaction Count',x='Quater',
                                                colorcolumn='Year', title='Year - Quater wise Transaction Count')
                    with col2:
                        VizEngine.multi_line_chart(df=selectstate_selecttype_year_quater_wise_total_transaction_amount, y='Transaction Amount',x='Quater',
                                                colorcolumn='Year', title='Year - Quater wise Transaction Amount')
                    ...
                    if option == 'Transaction Count':
                        ...
                        VizEngine.geo_map(selectyear_selecttype_state_wise_total_transaction_count,
                                       'State', 'Transaction Count', 'State wise Transaction Count')
                        ...
                        selectyear_selecttype_state_wise_total_transaction_amount = aggregated_transaction.selectyear_selecttype_state_wise_total_transaction_amount(
                            year_option, type_option)
                        VizEngine.geo_map(selectyear_selecttype_state_wise_total_transaction_amount,
                                       'State', 'Transaction Amount', 'State wise Transaction Amount')
                    ...
                    selectyear_selecttype_quater_wise_total_transaction_amount = aggregated_transaction.selectyear_selecttype_quater_wise_total_transaction_amount(
                                                                                                        year_option, type_option)
                    with col1:
                        VizEngine.vertical_bar_chart(df=selectyear_selecttype_quater_wise_total_transaction_count,x='Quater',y='Transaction Count', 
                                                    text='Transaction', color='#5D9A96', title='Quater wise Transaction Count')
                    with col2:
                        VizEngine.vertical_bar_chart(df=selectyear_selecttype_quater_wise_total_transaction_amount,x='Quater',y='Transaction Amount', 
                                                    text='Transaction', color='#5cb85c', title='Quater wise Transaction Amount')
                    ...
                    if option == 'Transaction Count':
                        ...
                        VizEngine.geo_map(selectquater_selecttype_state_wise_total_transaction_count,
                                       'State', 'Transaction Count', 'State wise Transaction Count')
                        ...
                        selectquater_selecttype_state_wise_total_transaction_amount = aggregated_transaction.selectquater_selecttype_state_wise_total_transaction_amount(
                            quater_option, type_option)
                        VizEngine.geo_map(selectquater_selecttype_state_wise_total_transaction_amount,
                                       'State', 'Transaction Amount', 'State wise Transaction Amount')
                    ...
                    selectquater_selecttype_year_wise_total_transaction_amount = aggregated_transaction.selectquater_selecttype_year_wise_total_transaction_amount(
                                                                                                        quater_option, type_option)
                    with col1:
                        VizEngine.vertical_bar_chart(df=selectquater_selecttype_year_wise_total_transaction_count,x='Year',y='Transaction Count', 
                                                    text='Transaction', color='#5D9A96', title='Year wise Transaction Count')
                    with col2:
                        VizEngine.vertical_bar_chart(df=selectquater_selecttype_year_wise_total_transaction_amount,x='Year',y='Transaction Amount', 
                                                    text='Transaction', color='#5cb85c', title='Year wise Transaction Amount')
                    ...
                    selectstate_selectyear_selecttype_quater_wise_total_transaction_amount = aggregated_transaction.selectstate_selectyear_selecttype_quater_wise_total_transaction_amount(
                                                                                                                    state_option, year_option, type_option)
                    with col1:
                        VizEngine.vertical_bar_chart(df=selectstate_selectyear_selecttype_quater_wise_total_transaction_count,x='Quater',y='Transaction Count', 
                                                    text='Transaction', color='#5D9A96', title='Quater wise Transaction Count')
                    with col2:
                        VizEngine.vertical_bar_chart(df=selectstate_selectyear_selecttype_quater_wise_total_transaction_amount,x='Quater',y='Transaction Amount', 
                                                    text='Transaction', color='#5cb85c', title='Quater wise Transaction Amount')
                    ...
                    selectstate_selectquater_selecttype_year_wise_total_transaction_amount = aggregated_transaction.selectstate_selectquater_selecttype_year_wise_total_transaction_amount(
                                                                                                                    state_option, quater_option, type_option)
                    with col1:
                        VizEngine.vertical_bar_chart(df=selectstate_selectquater_selecttype_year_wise_total_transaction_count,x='Year',y='Transaction Count', 
                                                    text='Transaction', color='#5D9A96', title='Year wise Transaction Count')
                    with col2:
                        VizEngine.vertical_bar_chart(df=selectstate_selectquater_selecttype_year_wise_total_transaction_amount,x='Year',y='Transaction Amount', 
                                                    text='Transaction', color='#5cb85c', title='Year wise Transaction Amount')
                    ...
                    if option == 'Transaction Count':
                        ...
                        VizEngine.geo_map(selectyear_selectquater_selecttype_state_wise_total_transaction_count,
                                       'State', 'Transaction Count', 'State wise Transaction Count')
                        ...
                        selectyear_selectquater_selecttype_state_wise_total_transaction_amount = aggregated_transaction.selectyear_selectquater_selecttype_state_wise_total_transaction_amount(
                            year_option, quater_option, type_option)
                        VizEngine.geo_map(selectyear_selectquater_selecttype_state_wise_total_transaction_amount,
                                       'State', 'Transaction Amount', 'State wise Transaction Amount')
                ...
                if option == 'Transaction Count':
                    selecttype_state_wise_total_transaction_count = aggregated_transaction.selecttype_state_wise_total_transaction_count(
                        type_option)
                    VizEngine.geo_map(selecttype_state_wise_total_transaction_count,
                                   'State', 'Transaction Count', 'State wise Transaction Count')

                elif option == 'Transaction Amount':
                    selecttype_state_wise_total_transaction_amount = aggregated_transaction.selecttype_state_wise_total_transaction_amount(
                        type_option)
                    VizEngine.geo_map(selecttype_state_wise_total_transaction_amount,
                                   'State', 'Transaction Amount', 'State wise Transaction Amount')
                ...
                with col1:
                    VizEngine.vertical_bar_chart(df=selecttype_year_wise_total_transaction_count,x='Year',y='Transaction Count', 
                                                text='Transaction', color='#5D9A96', title='Year wise Transaction Count')
                with col2:
                    VizEngine.vertical_bar_chart(df=selecttype_year_wise_total_transaction_amount,x='Year',y='Transaction Amount', 
                                                text='Transaction', color='#5cb85c', title='Year wise Transaction Amount')
                ...
                with col1:
                    VizEngine.pie_chart(df=selecttype_quater_wise_total_transaction_count, x='Quater', y='Transaction Count',
                                    title='Quater wise Transaction Count')
                with col2:
                    VizEngine.pie_chart(df=selecttype_quater_wise_total_transaction_amount, x='Quater', y='Transaction Amount',
                                    title='Quater wise Transaction Amount') 
                ...
                with col1:
                    VizEngine.multi_line_chart(df=selecttype_year_quater_wise_total_transaction_count, y='Transaction Count',x='Quater',
                                            colorcolumn='Year', title='Year - Quater wise Transaction Count')
                with col2:
                    VizEngine.multi_line_chart(df=selecttype_year_quater_wise_total_transaction_amount, y='Transaction Amount',x='Quater',
                                            colorcolumn='Year', title='Year - Quater wise Transaction Amount')
    
        elif analysis == 'User Brand':
            # vertical_bar chart
            brand_wise_total_user_count = aggregated_user.brand_wise_total_user_count()
            VizEngine.vertical_bar_chart(df=brand_wise_total_user_count,x='User Brand',y='User Count', 
            ...
            VizEngine.multi_line_chart(df=year_brand_wise_total_user_count, y='User Count',x='Year',
                                    colorcolumn='User Brand', title='Year - Brand wise User Count', height=600, title_x=0.42)
            VizEngine.multi_line_chart(df=quater_brand_wise_total_user_count, y='User Count',x='Quater',
            ...
            if brand_option and advanced_filters:
                if state_option != 'Select One' and year_option == 'Select One' and quater_option == 'Select One':
                    ...
                    with col1:
                        ...
                        VizEngine.vertical_bar_chart(df=selectstate_selectbrand_year_wise_total_user_count,x='Year',y='User Count', 
                    ...
                    with col2:
                        ...
                        selectstate_selectbrand_quater_wise_total_user_count = aggregated_user.selectstate_selectbrand_quater_wise_total_user_count(
                                                                                                state_option, brand_option)
                        VizEngine.pie_chart(df=selectstate_selectbrand_quater_wise_total_user_count, x='Quater', y='User Count',
                    ...
                    selectstate_selectbrand_year_quater_wise_total_user_count = aggregated_user.selectstate_selectbrand_year_quater_wise_total_user_count(
                                                                                                state_option, brand_option)
                    VizEngine.marker_multi_line_chart(df=selectstate_selectbrand_year_quater_wise_total_user_count, y='User Count',x='Quater', colorcolumn='Year',
                    ...
                    VizEngine.geo_map(selectyear_selectbrand_state_wise_total_user_count,
                                   'State', 'User Count', 'State wise User Count')

                    # vertical_bar chart
                    selectyear_selectbrand_quater_wise_total_user_count = aggregated_user.selectyear_selectbrand_quater_wise_total_user_count(
                                                                                            year_option, brand_option)
                    VizEngine.vertical_bar_chart(df=selectyear_selectbrand_quater_wise_total_user_count,x='Quater',y='User Count', 
                                                    text='User', color='#5D9A96', title='Quater wise User Count', title_x=0.40)
                    ...
                    VizEngine.geo_map(selectquater_selectbrand_state_wise_total_user_count,
                                   'State', 'User Count', 'State wise User Count')

                    # pie chart
                    selectquater_selectbrand_year_wise_total_user_count = aggregated_user.selectquater_selectbrand_year_wise_total_user_count(
                                                                                            quater_option, brand_option)
                    VizEngine.pie_chart(df=selectquater_selectbrand_year_wise_total_user_count, x='Year', y='User Count',
                    ...
                    VizEngine.vertical_bar_chart(df=selectstate_selectyear_selectbrand_quater_wise_total_user_count,x='Quater',y='User Count', 
                                                    text='User', color='#5D9A96', title='Quater wise User Count', title_x=0.40)
                    ...
                    VizEngine.pie_chart(df=selectstate_selectquater_selectbrand_year_wise_total_user_count, x='Year', y='User Count',
                                    title='Year wise User Count', title_x=0.37)
                    ...
                    VizEngine.geo_map(selectyear_selectquater_selectbrand_state_wise_total_user_count,
                                   'State', 'User Count', 'State wise User Count')
                ...
                selectbrand_state_wise_total_user_count = aggregated_user.selectbrand_state_wise_total_user_count(
                                                                            brand_option)
                VizEngine.geo_map(selectbrand_state_wise_total_user_count,'State', 'User Count', 'State wise User Count')

                col1, col2 = st.columns(2)
                with col1:
                    # vertical_bar chart
                    selectbrand_year_wise_total_user_count = aggregated_user.selectbrand_year_wise_total_user_count(brand_option)
                    VizEngine.vertical_bar_chart(df=selectbrand_year_wise_total_user_count,x='Year',y='User Count', 
                                                text='User', color='#5D9A96', title='Year wise User Count', title_x=0.30)
                with col2:
                    # pie chart
                    selectbrand_quater_wise_total_user_count = aggregated_user.selectbrand_quater_wise_total_user_count(brand_option)
                    VizEngine.pie_chart(df=selectbrand_quater_wise_total_user_count, x='Quater', y='User Count',
                                        title='Quater wise User Count')
                
                # multi line chart
                selectbrand_year_quater_wise_total_user_count = aggregated_user.selectbrand_year_quater_wise_total_user_count(brand_option)
                VizEngine.marker_multi_line_chart(df=selectbrand_year_quater_wise_total_user_count, y='User Count',x='Quater', colorcolumn='Year',
                                                title='Year - Quater wise User Count', title_x=0.35, text='User', textposition='top center')
                ...
                if year_option and quater_option:
                    ...
                    with states:
                        ...
                        selectyear_selectquater_state_wise_top10_transaction = top_transaction_and_user.selectyear_selectquater_state_wise_top10_transaction(
                                                                                                        year_option, quater_option)
                        VizEngine.top10_transaction_state_vertical_bar_chart(df=selectyear_selectquater_state_wise_top10_transaction,x='State',y='Transaction Amount', 
                                text='Amount', color='#5D9A96', title='Top 10 States (by volume)', title_x=0.42)
                    ...
                    with districts:
                        ...
                        if state_option == 'Over All':
                            ...
                            selectyear_selectquater_district_wise_top10_transaction = top_transaction_and_user.selectyear_selectquater_district_wise_top10_transaction(
                                                                                                                year_option, quater_option)
                            VizEngine.top10_transaction_district_vertical_bar_chart(df=selectyear_selectquater_district_wise_top10_transaction,x='District_State',y='Transaction Amount', 
                                    text='Amount', color='#5D9A96', title='Top 10 Districts (by volume)', title_x=0.45)
                            ...
                            selectstate_selectyear_selectquater_district_wise_top10_transaction = top_transaction_and_user.selectstate_selectyear_selectquater_district_wise_top10_transaction(
                                                                                                                            state_option, year_option, quater_option)
                            VizEngine.top10_transaction_district_vertical_bar_chart(df=selectstate_selectyear_selectquater_district_wise_top10_transaction,x='District',y='Transaction Amount', 
                                    text='Amount', color='#5D9A96', title='Top 10 Districts (by volume)', title_x=0.45)
                    ...
                    with pincodes:
                        ...
                        if state_option == 'Over All':
                            ...
                            selectyear_selectquater_pincode_wise_top10_transaction = top_transaction_and_user.selectyear_selectquater_pincode_wise_top10_transaction(
                                                                                                                year_option, quater_option)
                            VizEngine.top10_transaction_pincode_vertical_bar_chart(df=selectyear_selectquater_pincode_wise_top10_transaction,x='Pincode_District_State',y='Transaction Amount', 
                                    text='Amount', color='#5D9A96', title='Top 10 PINs (by volume)', title_x=0.42)
                            ...
                            selectstate_selectyear_selectquater_pincode_wise_top10_transaction = top_transaction_and_user.selectstate_selectyear_selectquater_pincode_wise_top10_transaction(
                                                                                                                            state_option, year_option, quater_option)
                            VizEngine.top10_transaction_pincode_vertical_bar_chart(df=selectstate_selectyear_selectquater_pincode_wise_top10_transaction,x='Pincode_District',y='Transaction Amount', 
                                    text='Amount', color='#5D9A96', title='Top 10 PINs (by volume)', title_x=0.42)
                ...
                if year_option and quater_option:
                    ...
                    with states:
                        ...
                        selectyear_selectquater_state_wise_top10_user = top_transaction_and_user.selectyear_selectquater_state_wise_top10_user(
                                                                                                    year_option, quater_option)
                        VizEngine.top10_user_vertical_bar_chart(df=selectyear_selectquater_state_wise_top10_user,x='State',y='User Count', 
                                text='User', color='#5D9A96', title='Top 10 States (by volume)', title_x=0.42)
                    ...
                    with districts:
                        ...
                        if state_option == 'Over All':
                            ...
                            selectyear_selectquater_district_wise_top10_user = top_transaction_and_user.selectyear_selectquater_district_wise_top10_user(
                                                                                                        year_option, quater_option)
                            VizEngine.top10_user_vertical_bar_chart(df=selectyear_selectquater_district_wise_top10_user,x='District_State',y='User Count', 
                                    text='User', color='#5D9A96', title='Top 10 Districts (by volume)', title_x=0.45)
                            ...
                            selectstate_selectyear_selectquater_district_wise_top10_user = top_transaction_and_user.selectstate_selectyear_selectquater_district_wise_top10_user(
                                                                                                        state_option, year_option, quater_option)
                            VizEngine.top10_user_vertical_bar_chart(df=selectstate_selectyear_selectquater_district_wise_top10_user,x='District',y='User Count', 
                                    text='User', color='#5D9A96', title='Top 10 Districts (by volume)', title_x=0.45)
                    ...
                    with pincodes:
                        ...
                        if state_option == 'Over All':
                            ...
                            selectyear_selectquater_pincode_wise_top10_user = top_transaction_and_user.selectyear_selectquater_pincode_wise_top10_user(
                                                                                                        year_option, quater_option)
                            VizEngine.top10_user_vertical_bar_chart(df=selectyear_selectquater_pincode_wise_top10_user,x='Pincode_District_State',y='User Count', 
                                    text='User', color='#5D9A96', title='Top 10 PINs (by volume)', title_x=0.42)
                            ...
                            selectstate_selectyear_selectquater_pincode_wise_top10_user = top_transaction_and_user.selectstate_selectyear_selectquater_pincode_wise_top10_user(
                                                                                                                    state_option, year_option, quater_option)
                            VizEngine.top10_user_vertical_bar_chart(df=selectstate_selectyear_selectquater_pincode_wise_top10_user,x='Pincode_District',y='User Count', 
                                    text='User', color='#5D9A96', title='Top 10 PINs (by volume)', title_x=0.42)
...
if option:
    if option == 'Data Collection':
        ...
        c1 = data()
        c2 = DataFetcher.aggregated_transaction()
        if c1==c2:
            data_overview()
        ...
        c2 = DataFetcher.aggregated_transaction()
        if c1==c2:
            DatabaseLoader.sql_table_creation()
            DatabaseLoader.data_migration()
        ...
        st.balloons()
def _ashwin_signature():
    """Small signature function to mark authorship. Returns a dict with metadata."""
    return {
        'author': 'Ashwin R S',
        'version': '1.0-ashwin',
        'timestamp_utc': '2025-09-02T11:31:18.570615Z'
    }
